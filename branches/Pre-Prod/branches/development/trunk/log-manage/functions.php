...
function parseInfoFromFile($file, $correct_lines, $headers) {
    ...
    foreach ($correct_lines as $row) {
        ...
        foreach ($log_columns as $column) {
            $count++;
            $line[$column] = isset($split_rows[$count]) ?$split_rows[$count]: null;      
        }
...
function buildTheLayout($parsed_log) {
    ...
    foreach ($parsed_log[1] as $column) {
        if(($column!='cs-username') && ($column!='s-port') ){
        $id_counter++;
        $width ='';
        if(($column=='date') && ($column=='time') && ($column=='s-ip')&& ($column=='cs-method')  ){
            $width= 'width:25px';
        }
        $html .= '<th style="'.$width.'">' . $column . '<br /><input  class="searchRequests" id="search_' . $id_counter . '" placeholder="Search..." /></th>';
        }
    }
    $html .= '</tr>';
    $html .= '</thead>';
    ...
    foreach ($parsed_log[0] as $request_id=>$request_value) {
        unset($parsed_log[0][$request_id]['cs-username']);
        unset($parsed_log[0][$request_id]['s-port']);
        if ($parsed_log[0][$request_id]['sc-status'] >= 200 && $parsed_log[0][$request_id]['sc-status'] <= 399) {
       //unset($parsed_log[0][$request_id]);
        }
    }
    $i=1;
    $hideArray = array('/','/index.css','/favicon.ico','/docview.php',
    '/bodypain.php','/gauge_img.php','/images/up_a.png','/images/down_a.png','/atiemailsend');
   
    $showArray = array('/patientassessment.php','/startvisit.php');
    // Time for the table body with the requests, loop through the parsed_log array
    foreach ($parsed_log[0] as $request=>$statuses) {
       
       // if (!in_array($statuses['cs-uri-stem'],$hideArray)) {
        if (in_array($statuses['cs-uri-stem'],$showArray)) {
